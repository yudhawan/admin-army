    ...
    return (
        <>
            ...
            <div className='lg:block hidden w-3/4 fixed top-0 right-0 h-20 lg:flex justify-between px-5 items-center bg-transparent backdrop-blur-sm'>
                <div className='text-black text-xl font-bold'>{(pathname==="/")?<>Dashboard</>:(pathname==="/users")?<>Users</>:(pathname==="/personil")?<>Personil</>:(pathname==="/database")?<>Database</>:(pathname==="/monitoring")?<>Monitoring</>:<>Laporan</>}</div>
