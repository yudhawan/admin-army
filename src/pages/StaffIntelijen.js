...
import {getDrive,downloadFile} from '../features/intelSlice'
function StaffIntelijen() {
    const dispatch = useDispatch()
    const {intel,loading} = useSelector(state=>state.intel)
      ...
      <div className='flex flex-wrap gap-4 w-full h-full border-t border-slate-200 py-2 lg:py-5 justify-center'>
            {
                ...
                intel?.filter(value=> value.originalName.toLowerCase().includes(search.toLowerCase())).map((items,index)=>{
