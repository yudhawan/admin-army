...
import {getDrive,downloadFile} from '../features/driveSlice'
function PirantiLunak() {
    const dispatch = useDispatch()
    const {drive,loading} = useSelector(state=>state.drive)
  ...
  return (
    <div className='flex flex-col space-y-2 lg:space-y-5 w-full'>
      ...
      <div className='flex flex-wrap gap-4 w-full h-full border-t border-slate-300 py-2 lg:py-5 justify-center'>
            {
                loading?
                ...
                drive?.filter(value=> value.originalName.toLowerCase().includes(search.toLowerCase())).map((items,index)=>{
