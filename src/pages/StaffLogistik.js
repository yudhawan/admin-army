...
import {getDrive,downloadFile} from '../features/logSlice'
function StaffLogistik() {
    const dispatch = useDispatch()
    const {log,loading} = useSelector(state=>state.log)
      ...
      <div className='flex flex-wrap gap-4 w-full h-full border-t border-slate-200 py-2 lg:py-5 justify-center'>
            {
                ...
                log?.filter(value=> value.originalName.toLowerCase().includes(search.toLowerCase())).map((items,index)=>{
