...
import {getDrive,downloadFile} from '../features/opsSlice'
function StaffOperasi() {
    const dispatch = useDispatch()
    const {ops,loading} = useSelector(state=>state.ops)
      ...
      <div className='flex flex-wrap gap-4 w-full h-full border-t border-slate-200 py-2 lg:py-5 justify-center'>
            {
                ...
                ops?.filter(value=> value.originalName.toLowerCase().includes(search.toLowerCase())).map((items,index)=>{
