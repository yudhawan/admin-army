...
import ChartBar from '../components/Materill/ChartBar'
import Maintenance from '../components/Materill/Maintenance'
function Materill() {
  const dispatch = useDispatch()
  const {materill} = useSelector(state=>state.materill)
  const [search,setsearch]=useState('')
  const [tgl,settgl] = useState('')
  const filterDate = (date)=> settgl(date)
  useEffect(()=>{
    dispatch(getData())
  },[])
  return (
    <div className='flex flex-col space-y-2 lg:space-y-4'>
      <div className='flex w-full space-x-4'>
        <Maintenance filterDate={filterDate} />
      </div>
      <ChartBar/>
      <div className='flex space-x-2 items-center'>
        ...
        <div className='text-gray-400 hidden lg:block'>{materill?.length} Users</div>
      </div>
      <div className='flex'>
        <table className='w-full text-left overflow-x-scroll'>
            ...
            <tbody className='overflow-y-auto w-full h-[65vh] flex flex-col bg-white rounded-br-md rounded-bl-md'>
              {
              materill?.find(val => tgl?val.waktu_perawatan==tgl:val )?materill?.filter(val => val.waktu_perawatan==tgl ).filter(val => val.nomor_kendaraan.toLowerCase().includes(search.toLowerCase()) | val.nama_barang.toLowerCase().includes(search.toLowerCase())).map((item,index)=><tr className="flex w-full lg:w-full items-center py-3 px-4 border-b border-gray-100 h-24 space-x-1" key={index+1}>
                <td className='h-auto w-[15vw] lg:w-10 '>{index+1}</td>
                <td className='h-auto w-[50vw] lg:w-52 text-sm'>{item.nama_barang}</td>
                <td className='h-auto w-[50vw] lg:w-44 text-sm'>{item.tahun_pengadaan}</td>
                <td className='h-auto w-[30vw] lg:w-44 text-sm uppercase'>{item.status}</td>
              ...
              materill?.filter(val => val.nomor_kendaraan.toLowerCase().includes(search.toLowerCase()) | val.nama_barang.toLowerCase().includes(search.toLowerCase())).map((item,index)=><tr className="flex w-full lg:w-full items-center py-3 px-4 border-b border-gray-100 h-24 space-x-1" key={index+1}>
                <td className='h-auto w-[15vw] lg:w-10 '>{index+1}</td>
                <td className='h-auto w-[50vw] lg:w-52 text-sm'>{item.nama_barang}</td>
                <td className='h-auto w-[50vw] lg:w-44 text-sm'>{item.tahun_pengadaan}</td>
                <td className='h-auto w-[30vw] lg:w-44 text-sm uppercase'>{item.status}</td>
